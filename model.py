...
class FoldingBlock(torch.nn.Module):
    def __init__(self, num_input_features, num_output_features):
        ...
        self.mlp1 = MLP([]) 000
        self.mlp2 = MLP([]) 000
        self.atten = Self_Attn() 000
    def Up_module(self,points,grid):
        ratio = self.output/self.input
    ...
    def Down_module(self, points,up_points):
        point_1 = torch.reshape(up_points, (self.output, int(points.points.size(dim=1)*points.points.size(dim=0)/self.output))).contiguous()
